#!/usr/bin/env python3
"""
Lightweight GLB -> chunked GLB -> B3DM -> georeferenced 3D Tiles set.

Purpose:
- Split a GLB into 2-3 chunks based on mesh units
- Convert each chunk GLB into B3DM with 3d-tiles-tools
- Write a tileset.json whose root.transform actually applies
  lon/lat/height and heading/pitch/roll to the whole tileset
...
- If the source GLB effectively contains only one mesh, it may still create
  only one chunk.
- Requires: numpy, trimesh, and 3d-tiles-tools available through npx
"""
...
def enu_axes(lon_deg: float, lat_deg: float) -> np.ndarray:
    """Return 3x3 rotation from local ENU to ECEF."""
    lon = math.radians(lon_deg)
    lat = math.radians(lat_deg)
    sin_lon = math.sin(lon)
    cos_lon = math.cos(lon)
...
def hpr_rotation_matrix(heading_deg: float, pitch_deg: float, roll_deg: float) -> np.ndarray:
    """
    Local ENU rotation.
    Approximation suitable for practical placement:
    - heading around Up (Z in ENU)
    - pitch around local Y (North)
    - roll around local X (East)
    """
...
def make_transform(
    ...
    # 3D Tiles transform is stored as a 16-element column-major array
...
def write_tileset_json(
    ...
    root_bounds = scene_bounds([trimesh.creation.box(extents=[1, 1, 1], transform=np.eye(4))])
    # Replace dummy with actual chunk bounds aggregation
...
def main(argv: Sequence[str]) -> int:
    ...
    if not args.skip_b3dm:
        tiles_dir.mkdir(parents=True, exist_ok=True)
        if shutil.which("npx") is None:
            print("npx not found. Install Node.js/npm or run with --skip-b3dm.", file=sys.stderr)
            return 2
    ...
    for index, group in enumerate(groups):
        ...
        chunk_infos.append(
            ChunkInfo(
                ...
                glb_path=chunk_glb,
                b3dm_path=raw_b3dm,
                aligned_b3dm_path=aligned_b3dm,
                bounds=bounds,
    ...
    for c in chunk_infos:
        target = c.aligned_b3dm_path or c.glb_path
